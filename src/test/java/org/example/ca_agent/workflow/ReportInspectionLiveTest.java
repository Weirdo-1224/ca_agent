...
@SpringBootTest
class ReportInspectionLiveTest {

    @Autowired
    private CompetitiveAnalysisGraph graph;

    @Test
    void generateAndSaveReport() throws Exception {
        TaskInputDTO input = new TaskInputDTO();
        input.setTaskId("report_inspect_" + System.currentTimeMillis());
        input.setTaskName("AI Coding Tools Report Inspection");
        ...
        Path jsonPath = outDir.resolve("report-draft.json");
